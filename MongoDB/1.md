...
writeConcern ：写文档的安全级别。

ordered：批量时是否需要按顺序写入，如果为true，则上一个文档写入失败，则退出。如果为false，则遇到失败继续写入。
...
只能保证单个文档的原子性，不能保证多个文档的原子性。只能通过事务来实现。
...
删除某个属性。删除了balance和info.branch属性。至于""，可以是随意值不影响。

```js
db.posts.update({},{$unset:{balance:"","info.branch":""}})
```
...
update({},{})   第一个{}用于筛选文档，并定位要删除的数组。 第二个{}，用$占位符找到对应的数组，并筛选出要pull的文档。
...
删除数组中的内嵌文档时，pullAll要求删除条件与原内嵌文档完全一致，pull只需要满足条件即可。（待验证）
...
items是一个文档数组，其内有一个locations也是一个文档数组，这时候想要删除掉locations数组的某个文档：（类似目录下的看板的某个报表，根据动态参数，去掉某个报表。）
...
```js
//$nor
db.test.find({'$nor':[{'title':'title1'},{'seq':'1'}]}) 
```
...
```js
//$exists
db.test.find({'title':{'$exist':true}})
```

```js
//$type  
db.test.find({'title':{'$type':['objectId','object','null']}})
```

- 数组操作符

$all ：匹配数组中要包含所有查询值的文档

$eleMatch ： 匹配数组文档中单个文档符合所有查询条件。而不是数组文档中多个文档匹配满足查询条件
...
db.foo.find().skip(10).sort({'x':1}).limit(1)。

- sort、limit、skip 永远是先sort，再skip，再limit。
...
除了文档主键外，其余字段只能全部选择包含，或者不包含。

- 如果字段是数组，可以指定只返回数组的某部分，或者符合条件的第一个元素部分。

```js
db.studeng.find({},{models:{$slide:0}}) //只返回models数组中的第一个
db.studeng.find({},{models:{$slide:-1}})//只返回models数组中的最后一个
db.studeng.find({},{models:{$slide:[0,5]}}) //只返回models数组中的前5个

db.studeng.find({},{contact:{$eleMatch:{$gt:'Alabama'}}}) //只返回contact数组中的满足$gt条件的第一个元素
```
...
writeconcern 、 acknowledged（安全写级别被启用）
...
问题：如果索引字段不存在，则自动为null。当第二个文档也是唯一性索引不存在，也自动为null。这时候会报错，因为有重复的null了。（即使索引键字段值为null）

- 稀疏性

使得唯一索引具有稀疏性，可以保存多篇缺失索引键的文档。

```js
createIndex({balance:1},{unique:true,sparse:true})
```
...
## 索引生存时间
 ...
 ```js
 db.test.createIndex({pdate:1},{expireAfterSeconds:20}) //文档只保留30s，之后删除
 ```

- 复合索引不具备生存时间特性。
- 数据库使用后台线程检测，因此删除操作具有一定延迟性。
