...
2. 有key，path，moutPath。path为卷挂在在容器的路径，相对于mountPath。
   - mountPath：/etc/config ；path：path/to/test ；path：app.properties。
   - 生成：/etc/config/app.properties（app.properties是文件）
   - 生成：/etc/config/path/to/test（test是文件）
3. 四个都存在，subPath筛选失败，如果存在则mountPath指定到文件名。
   ...
   - 四个都存在，subPath筛选失败，容器根据mountPath生成一个空目录/etc/config/app.properties
