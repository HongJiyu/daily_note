...
定义：kubernetes服务（service）为一组功能相同的pod提供单一不变的接入点：**提供一个固定的ip**，可以供外部访问，也可以供内部集群访问。可以通过服务访问外部，也可以通过服务访问集群内部。
...
创建kubia-service服务，这个服务暴露自己的80端口，关联到含有app:kubia标签的所有pod的8080端口。（这个yaml仅能生成一个对内的ip，外部无法访问到这个ip）
...
但是无法ping < serviceName> 这是因为服务的集群ip是一个虚拟ip，需要与服务端口结合才有意义。属于正常现象。
...
常规的服务：type: ClusterIP ，这种服务只能用于集群内部访问。

而NodePort，不仅可以通过服务内部集群ip访问NodePort服务，还可以通过任何**集群节点**的IP和预留节点端口访问NodePort服务。
...
spec:
  type: NodePort
  ports:
  - port: 80  # 这个nodeport服务的端口
    targetPort:8080  # 应用程序的端口（pod端口）
    nodePort: 30123  #节点的端口
  selector:
    app: kubia
```
...
这个负载均衡器是对nodeport的负载均衡，也就是生成一个对外ip，然后均衡每个节点的访问量。
...
![image-20220501215059422](E:\dailynote\docker\img\image-20220501215059422.png)

![image-20220501215111652](E:\dailynote\docker\img\image-20220501215111652.png)

**ingress控制器不会将请求转发给服务，而是选择一个pod**
